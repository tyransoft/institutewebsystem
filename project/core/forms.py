...
class EmployeeForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if payment_type in ['weekly', 'daily', 'hourly'] and cleaned_data.get('hourly_rate', 0) <= 0:
            self.add_error('hourly_rate', 'أجر الساعة مطلوب لهذا النوع من الموظفين')
...
class AttendanceForm(forms.ModelForm):
    class Meta:
        model = Attendance
        fields = '__all__'
        exclude = ['hours_worked', 'deduction_amount', 'created_at']
        widgets = {
...
class PayrollForm(forms.ModelForm):
    class Meta:
        model = Payroll
        fields = ['employee', 'payroll_type', 'period_start', 'period_end', 'notes']
        widgets = {
    ...
    def clean(self):
        ...
        if period_start and period_end and period_start > period_end:
            self.add_error('period_end', 'تاريخ النهاية يجب أن يكون بعد تاريخ البداية')
...
class PayrollPaymentForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['payroll', 'employee', 'amount', 'payment_date', 'payment_method', 'notes']
        widgets = {
